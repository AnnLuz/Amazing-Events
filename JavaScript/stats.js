...
function cargarTabla1(array, contendor) {
    ...
    trContenedor.innerHTML = `
        <td class="border border-danger-black text-center">${mayorAttendance.name}: ${mayorAttendance.assistance/mayorAttendance.capacity*100}%</td>
        <td class="border border-danger-black text-center">${menorAttendance.name}: ${menorAttendance.assistance/menorAttendance.capacity*100}%</td>
        <td class="border border-danger-black text-center">${mayorCapacidad.name}: ${mayorCapacidad.capacity}</td>`
...
//  arreglo de categorias unicas
    ...
    for(categoria of categorias){
        let trContenedor = document.createElement('tr')
        trContenedor.innerHTML = `
        <td class="border border-danger-black text-center">${categoria}</td>
        <td class="border border-danger-black text-center">$${calcularGanancias(array,categoria)}</td>
        <td class="border border-danger-black text-center">${calcularAsistencia(array,categoria)}%</td>`
